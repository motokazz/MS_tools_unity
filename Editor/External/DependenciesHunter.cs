    ...
    public class AllProjectAssetsReferencesWindow : EditorWindow
    {
        ...
        public class AnalysisSettings
        {
            public static class PrefsKeys
            {
                ...
                public const string TargetPaths = Prefix + "TargetPaths";
            }
            ...
            public void LoadFromEditorPrefs()
            {
                ...
                var pathsStr = EditorPrefs.GetString(PrefsKeys.TargetPaths, "");
                TargetPaths = string.IsNullOrEmpty(pathsStr) ? new List<string>() : pathsStr.Split(';').ToList();
            }
            ...
            public void SaveToEditorPrefs()
            {
                ...
                EditorPrefs.SetString(PrefsKeys.TargetPaths, string.Join(";", TargetPaths));
            }
        ...
        [MenuItem("MS_Tools/Externals/Dependencies Hunter")]
        ...
        private void PopulateUnreferencedAssetsList()
        {
            ...
            foreach (var mapElement in map)
            {
                ...
                // Target Paths Filter
                if (_analysisSettings.TargetPaths != null && _analysisSettings.TargetPaths.Count > 0)
                {
                    bool isTarget = false;
                    foreach (var target in _analysisSettings.TargetPaths)
                    {
                        if (assetPath.StartsWith(target + "/") || assetPath == target)
                        {
                            isTarget = true;
                            break;
                        }
        ...
        private void OnAnalysisSettingsGUI()
        {
            ...
            // --- 新規追加: Target Paths設定 ---
            GUIUtilities.HorizontalLine();
            EditorGUILayout.LabelField(new GUIContent("Target Folders to Analyze (Empty = Whole Project)",
                "分析結果として抽出する対象のフォルダを指定します。正確な依存関係を調べるため、プロジェクト全体のスキャンは行われますが、結果として表示されるのはこのリストのフォルダ内のアセットのみになります。"), EditorStyles.boldLabel);

            for (int i = 0; i < _analysisSettings.TargetPaths.Count; i++)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(_analysisSettings.TargetPaths[i]);
                if (GUILayout.Button("X", GUILayout.Width(25)))
                {
                    _analysisSettings.TargetPaths.RemoveAt(i);
                    _analysisSettings.SaveToEditorPrefs();
                    i--;
                }
                EditorGUILayout.EndHorizontal();
            ...
            {
                string absolutePath = EditorUtility.OpenFolderPanel("Select Target Folder", "Assets", "");
                if (!string.IsNullOrEmpty(absolutePath) && absolutePath.StartsWith(Application.dataPath))
                {
                    string relativePath = "Assets" + absolutePath.Substring(Application.dataPath.Length);
                    if (!_analysisSettings.TargetPaths.Contains(relativePath))
                    {
                        _analysisSettings.TargetPaths.Add(relativePath);
                        _analysisSettings.SaveToEditorPrefs();
                    }
                }
